...
#+begin_src emacs-lisp
  ...
  (setq MattLu/socks-proxy "127.0.0.1:7890")    ; SOCKS proxy
  (setq MattLu/remote-host-name "mac")
  (setq MattLu/socks5 nil)

  (defun MattLu/new-socks5-proxy ()
    (apply #'start-process "socks5-proxy"
           (format "*socks5-proxy-%s*" MattLu/remote-host-name)
           "ssh" (list "-ND" MattLu/socks-proxy MattLu/remote-host-name)))

  (defun proxy-socks-show ()
    "Show SOCKS proxy."
    (interactive)
    (if (process-live-p MattLu/socks5)
  ...
  (defun proxy-socks-enable ()
    "Enable SOCKS proxy."
    (interactive)
    (unless (process-live-p MattLu/socks5)
      (setq MattLu/socks5 (MattLu/new-socks5-proxy)))
    (require 'socks)
    (setq url-gateway-method 'socks
          socks-noproxy '("localhost"))
    (let* ((proxy (split-string MattLu/socks-proxy ":"))
           (host (car proxy))
           (port (string-to-number (cadr proxy))))
      (setq socks-server `("Default server" ,host ,port 5)))
    ;; (setenv "all_proxy" (concat "socks5://" MattLu/socks-proxy))
    (proxy-socks-show))
...
#+begin_src emacs-lisp
  (defun MattLu/internet-up-p (&optional host)
...
#+begin_src emacs-lisp
  (use-package gptel
    ...
    ;; (unless (MattLu/internet-up-p "google.com")
    (setq-default gptel-proxy
                  (format "socks5://%s" MattLu/socks-proxy))
    ...
    (gptel-api-key (lambda ()
                     (string-trim-right
                      (f-read-text "~/.ssh/gemini_api_key")))))
#+end_src
...
#+begin_src emacs-lisp
  (defun MattLu/quit-all ()
      (interactive)
      (if (eq last-command 'MattLu/quit-all)
  ...
  (global-set-key (kbd "C-g") 'MattLu/quit-all)
#+end_src
