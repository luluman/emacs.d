...
Format c-common code.
We should put it above eglot.
#+begin_src emacs-lisp
  ...
  (defun MattLu/clang-format-overrite-eglot ()
    (define-key
      (set (make-local-variable 'eglot-mode-map)
           eglot-mode-map) (kbd "C-c u") nil))

  (defun MattLu/clang-format-key ()
    (define-key c-mode-base-map (kbd "C-c u") 'MattLu/clang-format))

  (use-package clang-format
    :custom
    (clang-format-style "llvm")
    ;; this code des not work
    :bind (:map c-mode-base-map
                ("C-c u" . MattLu/clang-format))
    :hook (((c-mode c++-mode) . MattLu/clang-format-overrite-eglot)
           (c-initialization . MattLu/clang-format-key)))
#+end_src
