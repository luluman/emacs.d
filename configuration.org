...
#+begin_src emacs-lisp
  ...
  (defvar MattLu/choosing-theme 'doom-xcode)
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (use-package eglot
    ...
    (MattLu/test-then-add-eglot '(js-mode typescript-mode js-ts-mode) "typescript-language-server" "--stdio")
...
#+begin_src emacs-lisp
  (setq treesit-langs "/usr/local/lib/tree-sitter-langs")
  (use-package treesit
    :if (file-directory-p treesit-langs)
    :custom
    (treesit-extra-load-path `(,treesit-langs))
    (treesit-max-buffer-size (* 100 1024 1024))
    (enable-local-variables nil) ;; disable checking the ‘-*-’ line
    :config
    (defun MattLu/change-mode-to-treesit (mode-alist)
      (cl-loop for item in mode-alist
               do (cl-case (cdr item)
                    (c++-mode (setf (cdr item) 'c++-ts-mode))
                    (python-mode (setf (cdr item) 'python-ts-mode))
                    (c-mode (setf (cdr item) 'c-ts-mode))
                    (c-or-c++-mode (setf (cdr item) 'c++-ts-mode))
                    (javascript-mode (setf (cdr item) 'js-ts-mode))
                    (js-json-mode (setf (cdr item) 'json-ts-mode))
                    )))
    (MattLu/change-mode-to-treesit auto-mode-alist)
    (MattLu/change-mode-to-treesit interpreter-mode-alist)
    :mode
    ("\\.h\\.inc\\'" . c++-ts-mode)
    ("\\.cpp\\.inc\\'" . c++-ts-mode))
#+end_src
