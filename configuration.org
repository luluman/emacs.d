...
#+begin_src emacs-lisp
  (use-package ghostel
    :preface
    (defun my/ghostel ()
      "Open Ghostel, then enable line mode once the prompt has rendered."
      (interactive)
      (let ((buffer (ghostel t)))
        ;; Enabling line mode immediately would race with prompt rendering,
        ;; so defer it slightly to give Ghostel time to draw the shell prompt.
        (run-at-time 0.1 nil
                     (lambda (buf)
                       (with-current-buffer buf
                         (ghostel-line-mode)))
