...
#+begin_src emacs-lisp
  (setq MattLu/minor-mode-direct '(flymake-mode project-mode-line-tag-mode))
  (defun powerline-minor-modes-modify (orig-fun &rest args)
    ...
    (let ((minor-mode-alist
           (cl-remove-if-not (lambda (mode)
                             ...
                             (append minor-mode-alist mode-line-misc-info))))
      (apply orig-fun args)))
#+end_src
...
#+begin_src emacs-lisp
  (use-package project
    ...
    (when (executable-find "rg")
      (declare-function grep-read-files "rg"))
    (when (executable-find "fd")
      (setq find-program "fd")))
#+end_src

Provides a global minor mode to display information about a buffer’s project (a
“project tag”) in its mode line.

#+begin_src emacs-lisp
  (use-package project-mode-line-tag
    :config
    (add-to-list 'mode-line-misc-info
      '(project-mode-line-tag-mode (:eval (project-mode-line-tag)) " "))
...
#+begin_src emacs-lisp
  (defun MattLu/test-then-add-eglot (program mode-list &optional command)
...
#+BEGIN_SRC emacs-lisp
  (use-package eglot
    ...
    (MattLu/test-then-add-eglot "clangd" '(c-mode c++-mode))
    (MattLu/test-then-add-eglot "pyright-langserver" '(python-mode) "--stdio")
    (MattLu/test-then-add-eglot "texlab" '(tex-mode context-mode texinfo-mode bibtex-mode))
    (MattLu/test-then-add-eglot "cmake-language-server" '(cmake-mode))
    (MattLu/test-then-add-eglot "typescript-language-server" '(js-mode typescript-mode) "--stdio")
    (MattLu/test-then-add-eglot "bash-language-server" '(sh-mode) "start"))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package flyspell
    ...
    (flyspell-mode . flyspell-configure-jit-lock))
#+END_SRC
