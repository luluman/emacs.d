...
#+begin_src emacs-lisp
  (defun my/add-compilation-environment ()
    "Prepend `compilation-environment' to `process-environment'.
  This lets shells and other modes that honor `process-environment'
  pick up the same variables used during compilation.  Directory-local
  variables are applied first so the buffer reflects its `.dir-locals'."
...
#+begin_src emacs-lisp
  (defun my/eglot-register-server (modes contact &optional executable)
    "Register CONTACT for MODES and enable Eglot when its server is available.
      MODES is a major-mode symbol or a list of major-mode symbols. CONTACT is a
      command list or a function accepted by `eglot-server-programs'.

      EXECUTABLE names the program to check before starting Eglot. It
      defaults to the first element of a command list and must be supplied
      explicitly for a function-valued CONTACT."
    (let ((program (or executable (car-safe contact))))
      (unless (stringp program)
        (error "Specify an executable for Eglot contact %S" contact))
  ...
  (defun my/eglot-tblgen-server-program (_interactive)
    "Return the tblgen-lsp-server command line for the current project.
      The compilation database is looked up in the project's CMake build
      directory.  Suitable as a function entry in `eglot-server-programs'."
    (let ((database (expand-file-name "tablegen_compile_commands.yml"
  ...
  (defun my/flymake-toggle-diagnostics-at-end-of-line ()
    "Toggle Flymake end-of-line diagnostics between disabled and `fancy'.
      Flymake is restarted in the current buffer so the change takes effect
      immediately."
    (interactive)
...
#+BEGIN_SRC emacs-lisp
  (use-package eglot
    ...
    (eglot-menu-string "")
    :config
    ;; Allow projects to override server programs via .dir-locals.el.
    (put 'eglot-server-programs 'safe-local-variable #'listp)

    ;; Language servers
    (dolist (server
             '(((c-or-c++-mode c-mode c++-mode c-or-c++-ts-mode c++-ts-mode c-ts-mode) ("clangd"))
               ((cmake-ts-mode) ("neocmakelsp" "stdio"))
               ((python-mode python-ts-mode) ("ty" "server"))
               ((LaTeX-mode tex-mode context-mode texinfo-mode bibtex-mode) ("texlab"))
               ((javascript-mode js-ts-mode) ("typescript-language-server" "--stdio"))
               ...
               ((mojo-mode) ("mojo-lsp-server"))
               ;; TableGen needs a per-project command line, so it is registered as a
               ;; function rather than a static program list.
               ((tablegen-mode) my/eglot-tblgen-server-program "tblgen-lsp-server")))
      (apply #'my/eglot-register-server server)))
#+END_SRC
...
#+begin_src emacs-lisp
  (use-package eldoc-box
    :config
    ;; Reset the Eldoc child frame when the selected parent frame changes.
    (defun my/eldoc-box-reset-frame-on-parent-change (&rest _)
      (when eldoc-box--frame
        (unless (and (frame-live-p eldoc-box--frame)
                     (eq (frame-parent eldoc-box--frame)
                         (selected-frame)))
          (eldoc-box-reset-frame))))

    (advice-add #'eldoc-box--frame-visible-p
                :before #'my/eldoc-box-reset-frame-on-parent-change)

    (defun my/eldoc-box-quit-on-window-change (_frame)
      "Quit eldoc-box childframe when window selection changes."
      ;; Don't quit if the user clicked into the childframe itself.
      (unless (eq (selected-frame) eldoc-box--frame)
        (eldoc-box-quit-frame)
        (remove-hook 'window-selection-change-functions
                     #'eldoc-box--quit-on-window-change)))

    (add-hook 'window-selection-change-functions
              #'my/eldoc-box-quit-on-window-change)
...
#+begin_src emacs-lisp
  (use-package dape
    ...
    :hook
    (dape-start . my/add-compilation-environment))
#+end_src
...
#+begin_src emacs-lisp
  (use-package vterm
    :preface
    (defun my/vterm-pop-to-buffer-same-window (buffer)
      ...
      (with-current-buffer buffer
        (unless (derived-mode-p 'vterm-mode)
          ...
          (let ((process-environment
                 (append compilation-environment process-environment)))
            (vterm-mode)
            (when (executable-find "xonsh")
              (vterm-send-string "xonsh\n"))))))

    (defun my/vterm ()
      "Open a vterm session in the current window."
      (interactive)
      (vterm--internal #'my/vterm-pop-to-buffer-same-window))

    :commands (vterm--internal)
    :bind ("C-c v" . my/vterm) ;; terminal
    :hook
    (vterm-mode . (lambda ()
                    (hl-line-mode -1)
...
#+begin_src emacs-lisp
  (use-package eat
    ...
    (defun my/eat-add-compilation-environment ()
      (my/add-compilation-environment)
      (corfu-mode t)
      (hl-line-mode -1)
      (setq-local global-hl-line-mode nil)
...
#+begin_src emacs-lisp
  (use-package ghostel
    :preface
    (defun my/ghostel ()
      "Open Ghostel, then switch to line mode after the prompt renders."
      (interactive)
    ...
    :hook
    (ghostel-mode . my/add-compilation-environment)
    :bind
    ("C-x m" . my/ghostel)
    :config
...
#+begin_src emacs-lisp
  (defun my/treesit--last-leaf (node)
    "Return the deepest last descendant of NODE, or NODE itself if it is a leaf."
    (let (child)
      (while (setq child (treesit-node-child node -1))
        (setq node child)))
    node)

  (defun my/treesit--nearest-sibling (node step)
    "Return the nearest real sibling of NODE, walking with STEP."
    (let (found)
      (while (and node (not found))
        (let ((sibling (funcall step node)))
          (cond ((null sibling)
                 (setq node (treesit-node-parent node)))
                ((treesit-node-check sibling 'missing)
                 (setq node sibling))
                (t
                 (setq found sibling)))))
      found))

  (defun my/treesit-forward-node ()
    "Move point to the start of the next node in pre-order."
    (interactive)
    (when-let* ((node (treesit-node-at (point)))
                (target (my/treesit--nearest-sibling node #'treesit-node-next-sibling)))
      (goto-char (treesit-node-start target))))

  (defun my/treesit-backward-node ()
    "Move point to the start of the previous node in pre-order.
  The previous node is the last leaf of the preceding sibling subtree."
    (interactive)
    (when-let* ((node (treesit-node-at (point)))
                (target (my/treesit--nearest-sibling node #'treesit-node-prev-sibling)))
      (goto-char (treesit-node-start (my/treesit--last-leaf target)))))

  (defvar-keymap my/treesit-nav-mode-map
    :doc "Keymap for `my/treesit-nav-mode'."
    "M-f" #'my/treesit-forward-node
    "M-b" #'my/treesit-backward-node)

  (define-minor-mode my/treesit-nav-mode
    "Move over syntax nodes instead of words."
    :lighter " ts-nav"
    :keymap my/treesit-nav-mode-map)

  (defun my/treesit-nav-mode-maybe ()
    "Enable `my/treesit-nav-mode' when the buffer has a treesit."
    (when (treesit-parser-list)
      (my/treesit-nav-mode 1)))
#+end_src
...
#+begin_src emacs-lisp
  (use-package treesit
    ...
    :custom
    ;; Parse files up to 100 MiB; the default is too small for generated code.
    (treesit-max-buffer-size (* 100 1024 1024))
    ;; Prefer the tree-sitter variant of every mode that has one.
    (treesit-enabled-modes t)
    (treesit-auto-install-grammar t)

    :mode
    ;; TableGen/LLVM-style generated headers and CUDA headers are C++.
    ("\\.h\\.inc\\'" . c++-ts-mode)
    ("\\.cpp\\.inc\\'" . c++-ts-mode)
    ("\\.su\\'" . c++-ts-mode)

    :hook (prog-mode . my/treesit-nav-mode-maybe))
#+end_src
  ...
  (use-package gptel
    ...
    (setq gptel--openai
          (gptel-make-openai "CC"
            :host "aiapi.siorigin.com"
            :endpoint "/v1/chat/completions"
            :key 'gptel-api-key
            :stream t
            :models
            '(;; ZJ
              (Kimi-K3 :request-params (:reasoning_effort "max"))
              (model-a :request-params (:reasoning_effort "max"))
              (model-b :request-params (:reasoning_effort "max"))
              ;; DMX
              (deepseek-v4.1-flash :request-params (:reasoning_effort "max"))
              (kimi-k3 :request-params (:reasoning_effort "high"))
              (qwen3.8-max-0902 :request-params (:reasoning_effort "high"))
              (glm-5.3 :request-params (:reasoning_effort "high"))
              (deepseek-v4-pro-0813 :request-params (:reasoning_effort "high"))
              (gpt-5.6-sol :request-params (:reasoning_effort "high"))
              (gpt-6-astra :request-params (:reasoning_effort "high"))
              (claude-opus-5
               :request-params (:output_config (:effort "xhigh") :max_tokens 128000))
              (claude-fable-5
               :request-params (:output_config (:effort "xhigh") :max_tokens 128000)))))
    ...
    (defvar gptel--gemini
      (gptel-make-gemini "Gemini"
        ...
        :request-params '(:generationConfig (:thinkingConfig (:thinkingLevel "HIGH" :includeThoughts t)))))
    ...
    (defvar gptel--openrouter
      (gptel-make-openai "OpenRouter"
        ...
        :key 'gptel-api-key
        :models
        '((openai/gpt-6-astra-pro :request-params (:reasoning (:effort "xhigh")))
          (~openai/gpt-astra-latest :request-params (:reasoning (:effort "high")))
          (~moonshotai/kimi-latest :request-params (:reasoning (:effort "max")))
          (~z-ai/glm-latest :request-params (:reasoning (:effort "max")))
          (~anthropic/claude-fable-latest :request-params (:reasoning (:effort "high")))
          (~anthropic/claude-opus-latest :request-params (:reasoning (:effort "xhigh")))
          (~openai/gpt-latest :request-params (:reasoning (:effort "xhigh")))
          (~x-ai/grok-latest :request-params (:reasoning (:effort "max")))
          (~google/gemini-flash-latest :request-params (:reasoning (:effort "xhigh")))
          (~google/gemini-pro-latest :request-params (:reasoning (:effort "xhigh"))))))
    ...
    (setq-default gptel-backend gptel--deepseek)
...
#+begin_src emacs-lisp
  ...
  ;; NOTE: fuz is known to conflict with fzf-native, causing crashes.
  ;; Therefore, fuz is disabled and fzf-native is used as the backend for fussy.
  (use-package fuz
    :disabled
  ...
  (use-package fussy
    :config
    (fussy-eglot-setup)
    (fussy-setup-fzf)                     ; uses fzf-native as the matching engine
    ;; (fussy-setup-fuz)                  ; alternative if fuz were enabled
...
#+begin_src emacs-lisp
  (use-package embark
    ...
    (defvar-keymap gptel-model-map
      ...
      "p" (my/define-embark-gpt-refactor gptel--openai gpt-5.6-sol "")
      "o" (my/define-embark-gpt-refactor gptel--openai claude-opus-5 "")
      "i" (my/define-embark-gpt-refactor gptel--openai claude-fable-5 "")
      "u" (my/define-embark-gpt-refactor gptel--openai gpt-6-astra)
      "z" (my/define-embark-gpt-refactor gptel--openai glm-5.3 "")
      "d" (my/define-embark-gpt-refactor gptel--openai deepseek-v4-pro-0813 "")
      "q" (my/define-embark-gpt-refactor gptel--openai qwen3.8-max-0902 "")
      ;; ZJ
      "k" (my/define-embark-gpt-refactor gptel--openai Kimi-K3 "")
      "a" (my/define-embark-gpt-refactor gptel--openai model-a "")
      "b" (my/define-embark-gpt-refactor gptel--openai model-b "")

      "P" (my/define-embark-gpt-refactor gptel--openrouter ~openai/gpt-latest)
      "O" (my/define-embark-gpt-refactor gptel--openrouter ~anthropic/claude-opus-latest)
      "I" (my/define-embark-gpt-refactor gptel--openrouter ~anthropic/claude-fable-latest)
      "U" (my/define-embark-gpt-refactor gptel--openrouter ~openai/gpt-astra-latest)
      "K" (my/define-embark-gpt-refactor gptel--openrouter ~moonshotai/kimi-latest)
      "Z" (my/define-embark-gpt-refactor gptel--openrouter ~z-ai/glm-latest)
      "g" (my/define-embark-gpt-refactor gptel--openrouter ~google/gemini-flash-latest)
      "x" (my/define-embark-gpt-refactor gptel--openrouter ~x-ai/grok-latest)
...
#+begin_src emacs-lisp
  (use-package pyim
    ...
    (pyim-indicator-modeline-string (list "PY/C " "PY/E "))
...
#+begin_src emacs-lisp
  (use-package ts-movement
    ...
    :hook (my/treesit-nav-mode . ts-movement-mode))
#+end_src
