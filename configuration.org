...
#+begin_src emacs-lisp
  (use-package embark
    ...
    (setq my/gpt-refacotr-prompt "You are a professional programmer. Refactor the following code. Generate only code, no explanation, no code fences. Also rephrase the comment and rename the variable if needed.")
