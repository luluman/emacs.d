...
#+begin_src emacs-lisp
  (defun MattLu/clangd-cleanup ()
    (mapc (lambda (x) (delete-file x 'always))
          (directory-files "/tmp/" t "\\(.+\\.pch\\)\\|\\(symbolizer-+.+\\)"))
    (message "clean up clangd!"))

  (defun MattLu/lsp-cleanup ()
    (if (eglot-managed-p)
        (setq-local clangd-cleanup--timer-objects
                    (run-with-idle-timer 3600 t 'MattLu/clangd-cleanup))
...
#+BEGIN_SRC emacs-lisp
  (use-package eglot
    ...
    (((c-mode c++-mode python-mode sh-mode typescript-mode)
      . eglot-ensure)
     (eglot-managed-mode . MattLu/lsp-cleanup)))
#+END_SRC
