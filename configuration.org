  ...
  (defun powerline-minor-modes-modify (orig-fun &rest args)
    ...
    (let ((minor-mode-alist
           (cl-remove-if-not (lambda (mode)
                               (memq (car mode) my/minor-mode-direct))
                             minor-mode-alist)))
      (apply orig-fun args)))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (use-package eglot
    ...
    :config
    (my/test-then-add-eglot '(c-mode c++-mode c++-ts-mode c-ts-mode) "clangd")
    ...
    (my/test-then-add-eglot '(js-ts-mode) "typescript-language-server" "--stdio")
    (my/test-then-add-eglot '(bash-ts-mode) "bash-language-server" "start")
...
#+begin_src emacs-lisp
  (use-package gptel
    ...
    (gptel-make-openai "ChatGPT"
      :key (+get-api-key "~/.ssh/gpt_api_key")
      :stream t
      :models '("gpt-3.5-turbo" "gpt-3.5-turbo-16k" "gpt-4" "gpt-4o"
                "gpt-4-turbo" "gpt-4-turbo-preview" "gpt-4-32k"
                "gpt-4-1106-preview" "gpt-4-0125-preview")) ;;  bug

    (defvar gptel--gemini
      (gptel-make-gemini "Gemini" :stream t :key (+get-api-key "~/.ssh/gemini_api_key"))) ;; free
    (defvar gptel--anthropic
      (gptel-make-anthropic "Claude" :stream t :key (+get-api-key "~/.ssh/claude_api_key")))
