    ...
    (ef-themes-with-colors
      (if (equal "ef-melissa-light" (symbol-name (car custom-enabled-themes)))
          (custom-set-faces
           ...
           `(font-lock-comment-delimiter-face ((,c :inherit italic :foreground "grey"))))
        (custom-set-faces
         `(font-lock-comment-face ((,c :inherit italic :foreground ,fg-dim)))
         `(font-lock-comment-delimiter-face ((,c :inherit italic :foreground ,fg-dim)))))
      ))
...
#+begin_src emacs-lisp
  (use-package gptel
    ...
    (defvar gptel--deepseek-speciale
      (gptel-make-deepseek "DeepSeek-Speciale"
        :stream t
        :endpoint "/v3.2_speciale_expires_on_20251215/chat/completions"
        :key 'gptel-api-key
...
#+begin_src emacs-lisp
  (use-package embark
    ...
    (defvar-keymap gptel-openrouter-model-map
      ...
      "o" (my/define-embark-gpt-refactor gptel--openrouter openai/gpt-5.1)
      "i" (my/define-embark-gpt-refactor gptel--openrouter openai/gpt-5)
      "p" (my/define-embark-gpt-refactor gptel--openrouter anthropic/claude-opus-4.5)
      "c" (my/define-embark-gpt-refactor gptel--openrouter openai/gpt-5.1-codex)
      "x" (my/define-embark-gpt-refactor gptel--openrouter openai/gpt-5-codex)
      "m" (my/define-embark-gpt-refactor gptel--openrouter openai/gpt-5.1-codex-max)
      "s" (my/define-embark-gpt-refactor gptel--openrouter anthropic/claude-sonnet-4.5))
    ...
    (keymap-set embark-region-map "d" (my/define-embark-gpt-refactor gptel--deepseek-speciale))
...
#+begin_src emacs-lisp
  (use-package corfu
    ...
    ;; Explicitly delete the Corfu popup after quitting,
    ;; as it can occasionally linger when switching buffers.
    (advice-add 'corfu-quit :after
                (lambda () (when corfu--frame (delete-frame corfu--frame))))
