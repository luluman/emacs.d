...
#+BEGIN_SRC emacs-lisp
  (set-face-background 'default "nord3")
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package rtags
    :config
;;    (set-variable 'rtags-path (file-truename "~/Matt_Installed_package/rtags/build/bin"))
...
#+BEGIN_SRC emacs-lisp
  (setq python-shell-interpreter (file-truename "~/EnvPy/my_py/bin/ipython3"))
...
Using anaconda as python backend
#+BEGIN_SRC emacs-lisp
  (defun MattLu/annotation-function (candidate)
    (let ((description (get-text-property 0 'description candidate)))
      (when description
        (concat "<" (substring description 0 1) ">"))))

  (setq company-anaconda-annotation-function 'MattLu/annotation-function)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package ace-pinyin
    :bind ("C-c j" . ace-pinyin-jump-char)
    :config
...
;    (setq aw-background nil) ;turn off the gray background
...
#+BEGIN_SRC emacs-lisp
  (global-set-key (kbd "M-s") 'select-frame-by-name)
  (global-set-key (kbd "C-c n") 'set-frame-name)
#+END_SRC
